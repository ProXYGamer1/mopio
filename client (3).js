...
if (KTestingModeON) {
  ...
  reg = "LOCAL";
  //addServerDef("LOCAL", "b3ef-92-51-30-5.eu.ngrok.io", reg);
...
function gameServerConnect(newGameServer, autoClickPlay = false) {
  ...
  var wsPort = wsProtocol == "ws://" ? 7020 : 7020;

  var conUrl = wsProtocol + curServer.serverConnURL + (curServer.serverConnURL.includes("ngrok") ? "" : ":" + wsPort);
...
GameObj.prototype.worldUpd_readMsgNewlyVisible = function (
  ...
  if (f_rectObj) {
    ...
    console.log(this.rectW, this.rectH)
  }
...
Animal.prototype.drawUnderSkinImgOutline = function () {
  ...
  if (
    ...
    (this.animalType == a_deer ||
      /*this.animalType == a_zebra || */
      this.animalType == a_reindeer)
  ) {
    //console.log("dig");
...
Animal.prototype.readCustomData_onUpdate = function (msg) {
  ...
  this.curBiome = aniFlags.getInt0to3();
  // if (aniFlags == 1) {
  //   this.curBiome = 1
  // } else if (aniFlags == 2) {
  //   this.curBiome = 2
...
function fillGrid(x1, y1, x2, y2, curRelX, curRelY) {
  ...
  for (
    dy = -0.5 + y1 + ((oldFillH - y1) % gSize); dy < y1 + fillH; dy += gSize
  ) {
    ctx.beginPath();
    ctx.moveTo(x1, dy);
...
function toggleChatOpen() {
  if (!isDevMode)
    if (_gameMode != null && !_gameMode.chatAllowed) return;
